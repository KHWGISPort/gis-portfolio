# -*- coding: utf-8 -*-
"""
[Phase 5 - 보완] 마스킹 규칙이 반영된 v2 결과로 웹지도를 다시 만든다.
(22번 스크립트의 마스킹 이전 지도는 비교용으로 그대로 남겨둔다.)
...
QUADRANT_COLOR = {
    ...
    "출점불가(시가지아님)": "#4d3b2a",         # 짙은 갈색 - 마스킹(산지/미개발지 추정)
}

if __name__ == "__main__":
    grid = gpd.read_file("data/processed/jungnang_scored_grid_v2.gpkg", layer="grid")
    ...
    for q in QUADRANT_COLOR:
        # 마스킹된 '출점불가'는 기본적으로 지도를 열자마자 보이지 않게(꺼둠) 해서
        # 우선 진짜 후보지들이 눈에 잘 띄게 한다 (필요하면 레이어 컨트롤에서 켤 수 있음)
        show_default = q != "출점불가(시가지아님)"
    ...
    <div style="position: fixed; bottom: 30px; left: 30px; z-index:9999;
      ...
      <span style="color:#4d3b2a;">■</span> 출점불가 (반경 300m 내 상가 0개, 시가지 아님)<br>
    ...
    out_path = "outputs/25_jungnang_map_masked.html"
    m.save(out_path)
    print(f"저장 완료: {out_path}")
    print(f"격자 수: {len(grid_4326)}, 지도 내 편의점 수: {len(stores_jn)}")
    print(f"마스킹된 격자 수: {(grid_4326['quadrant'] == '출점불가(시가지아님)').sum()}")
