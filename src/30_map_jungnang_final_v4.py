# -*- coding: utf-8 -*-
"""
[Phase 5 - 최종 완성] 건물기반+업소기반 결합 마스킹(v4)이 반영된 최종 웹지도.
...
QUADRANT_COLOR = {
    ...
    "출점불가(건물+업소결합)": "#4d3b2a",      # 짙은 갈색 - 마스킹(도로/하천/산지/공원체육시설 등)
}
...
if __name__ == "__main__":
    ...
    out_path = "outputs/30_jungnang_map_final_v4.html"
    m.save(out_path)
    print(f"저장 완료: {out_path}")
    print(f"격자 수: {len(grid_4326)}, 지도 내 편의점 수: {len(stores_jn)}")
    print(f"마스킹된 격자 수: {(grid_4326['quadrant'] == '출점불가(건물+업소결합)').sum()}")
